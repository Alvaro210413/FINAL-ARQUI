...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            # Procesar imagen con GaussianBlur
            start = time.time()
            procesada = procesar_blur(payload)
            dur_ms = int((time.time() - start) * 1000)

            # Guardar imagen en results/
            ts = time.strftime("%Y%m%d_%H%M%S")
            base_name = os.path.splitext(os.path.basename(nombre))[0]
            out_name = f"C_{base_name}_{ip_sim.replace('.', '-')}_{ts}.jpg"
            out_path = os.path.join("results", out_name)

            with open(out_path, "wb") as f:
                f.write(procesada)

            print(f"Imagen procesada (GaussianBlur) guardada en: {out_path} ({dur_ms} ms)")
